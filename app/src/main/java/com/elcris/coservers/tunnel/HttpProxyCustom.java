		...
		implements ProxyData
{
	...
	public HttpProxyCustom(String proxyHost, int proxyPort, String proxyUser, String proxyPass, Context context) {
		this(proxyHost, proxyPort, proxyUser, proxyPass, null, false, context, false);
	}

	public HttpProxyCustom(String proxyHost, int proxyPort, String proxyUser, String proxyPass, String requestPayload, boolean modoDropbear, Context context, boolean replace) {
		if (proxyHost == null) {
	...
	public Socket openConnection(String hostname, int port, int connectTimeout, int readTimeout) throws IOException {
		...
		if (len != 101) {
            if (autoreplace) {
			    ...
			    String replacw = httpReponseFirstLine.replace(httpReponseFirstLine, "HTTP/1.1 101 Ok");
		    	...
		    	len = Integer.parseInt(asw.replace(asw, "101"));
            }
		} else {
			return sock;
		}
		...
		if (len < 0 || len > 999) {
			...
			String stringBuffer = new StringBuffer().append(new StringBuffer().append("HTTP/1.0 101 Connection established").append("\\").toString()).append("\\").toString();
