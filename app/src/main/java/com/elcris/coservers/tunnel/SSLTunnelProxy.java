	...
	private SSLSocket doSSLHandshake(String host, String sni, int port) throws IOException {
		...
		try {
			...
			if (BuildConfig.DEBUG) {
				//MOSTRA PAYLOAD
				SkStatus.logInfo("SNI hostname: " + sni);
			...
			IOException iOException = new IOException(new StringBuffer().append("No fue posible concluir conexión SSL handshake: ").append(e).toString());
			throw iOException;
		}
	}
