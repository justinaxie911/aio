...
public class Tunnel
{
	...
	{
		StringBuilder routeMessage = new StringBuilder("Rutas: ");
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			mPdnsd.setOnPdnsdListener(new Pdnsd.OnPdnsdListener(){
				@Override
				public void onStart(){
					//mHostService.onDiagnosticMessage("Servicio DNS Iniciado");
				}
				@Override
				public void onStop(){
					//mHostService.onDiagnosticMessage("Servicio DNS Parado");
					stop();
				}
			});
	...
	private void stopVpn()
	{
		...
		if (tunFd != null)
		{
			try
			{
				mHostService.onDiagnosticMessage("<font color=\"#ff0000\">Desconectado</font>");
				//TunnelManagerThread.jota();
				tunFd.close();
